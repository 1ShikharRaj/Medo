...
import { LayoutDashboard, Users, Clock, Stethoscope, FileCheck, Search, Bell } from 'lucide-react';
      ...
      <aside className="w-64 bg-card border-r border-border hidden md:flex flex-col fixed h-full z-20 shadow-sm">
        ...
        <div className="px-6 py-5 border-b border-border bg-slate-50/50">
          ...
          <div className="text-sm font-semibold text-foreground flex items-center gap-2 bg-white px-3 py-2 rounded-lg border border-slate-200 shadow-sm">
            <div className="w-2.5 h-2.5 rounded-full bg-blue-500 shadow-sm"></div> Health Worker
          </div>
        </div>
        
        <nav className="flex-1 py-6 px-4 space-y-1">
          <Link href="/health-worker" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-muted-foreground hover:bg-slate-100 hover:text-foreground font-medium transition-colors">
            <LayoutDashboard className="w-5 h-5" /> Dashboard
          </Link>
          <Link href="/health-worker/patients" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-muted-foreground hover:bg-slate-100 hover:text-foreground font-medium transition-colors">
            <Users className="w-5 h-5" /> Patients
          </Link>
          <Link href="/health-worker/follow-ups" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-muted-foreground hover:bg-slate-100 hover:text-foreground font-medium transition-colors">
            <Clock className="w-5 h-5" /> Follow-ups
          </Link>
          <Link href="/health-worker/completed" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-muted-foreground hover:bg-slate-100 hover:text-foreground font-medium transition-colors">
